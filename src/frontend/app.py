...
def _render_segmentation(data: dict, key_prefix: str = ""):
    """渲染分层结果"""
    ...
    st.plotly_chart(fig, use_container_width=True, key=f"{key_prefix}seg_chart")
...
def process_message(prompt: str):
    """发送消息到后端 — 只做 API 调用，渲染交给 Chat Display 循环"""
    ...
    with st.chat_message(msg["role"]):
        ...
        if role == "user":
            ...
            if rtype == "segmentation":
                ...
                _render_segmentation(data, key_prefix=key)
